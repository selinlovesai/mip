...
| Item | Type | What it Does | How to Migrate | Prerequisites | Complexity | Status | Success Criteria |
|---|---|---|---|---|---|---|---|
| FastAPI backend | Service | Chat + endpoint-test proxy + **generic DB CRUD** (`/api/db/{collection}`) | Done (`server/`); chat + test + CRUD live; expand to typed/secret routes | Postgres | M | 🟡 (chat + test + CRUD) | All data flows through backend |
| Postgres + ORM layer | Service | DB access (SQLAlchemy async + asyncpg) — generic `records(collection,id,data jsonb)` store, auto-created on startup, graceful degrade when down | Done (foundation); add typed `tokens`/`components` tables + migrations + seed (mirror `@mip/db`) | Postgres instance | L | 🟡 (generic store + CRUD verified) | CRUD round-trips persist; typed tables + seed next |
