...
> **Last updated — 2026-05-27.** Recent shipped work (beyond the original table): the **widget editor** reached full legacy parity in the slideout (data-source select incl. **custom one-off REST source**, request editor with **Test / Load fields**, structured field mapping, **per-column format + animation**, dedicated **content editors** for markdown/diagram/image, collapsible custom + advanced settings, move-to-page, delete) **plus a Design tab** (Figma-style per-element inspector). Every widget now has a **citation + Read-more footer** (source link + link-to-page navigation). The **AI assistant** is fully live: real inference, a tool loop that injects/edits widgets, web search (Tavily), content-level awareness of existing widgets, and guardrails against fabricated stats, duplicate-metric widgets, narration-without-action, and lost streamed replies. Plus a **"write my system prompt with AI"** button in Dashboard Settings. Statuses below reflect this.
...
|---|---|---|---|---|---|---|---|
| Color tokens | Foundation (Token) | Brand ramp + semantic colors driving every surface | Move Untitled `@theme` colors into a `tokens` table (name, value, mode, group); emit CSS vars from DB at boot; edit via Settings → Appearance | DB + token API | L | ✅ (501 color rows seeded from theme.css; `GET/PUT /api/tokens`; Appearance picker → PUT → live `:root` overlay; `bg-*`/`text-*` utilities read the vars) | Editing a color in Appearance writes to DB and recolors live; no hardcoded hex |
| Typography tokens | Foundation (Token) | Font families + display/text scale | Tokenize into `tokens` (group=typography); components reference token names; edit via Settings → Appearance | tokens table | M | ✅ (29 typography rows; fonts + type scale editable in Appearance → DB → live; `text-*` utilities resolve `var(--text-*)`) | Type scale editable from DB **and** from Settings → Appearance UI |
| Shadow tokens | Foundation (Token) | Elevation scale | Tokenize shadows; edit via Settings → Appearance | tokens table | S | ✅ (14 shadow rows editable; runtime overlay re-points `shadow-*` utilities at `var(--shadow-*)` since Tailwind inlines them) | Shadows editable from DB **and** from Settings → Appearance UI |
| Spacing tokens | Foundation (Token) | Spacing/gutter scale | Tokenize spacing; edit via Settings → Appearance | tokens table | M | ✅ (`--spacing` base unit editable → rescales the whole `calc(var(--spacing)*N)` scale live) | Spacing propagates from DB; editable from Settings → Appearance UI |
| Radius tokens | Foundation (Token) | Corner-radius scale | Tokenize radius; edit via Settings → Appearance | tokens table | S | ✅ (10 radius rows editable; `rounded-*` utilities resolve `var(--radius-*)`) | Radius editable from DB **and** from Settings → Appearance UI |
| Appearance token browser | Pattern / Block | Categorized token UI (Colors/Type/Shadows/Spacing) — the editing surface for every token | Done; each token value editable inline + persisted to `tokens` | tokens table | M | ✅ (data-driven from `/api/tokens`: Colors fan out into per-group sub-tabs; **all kinds editable** — color picker, typography/shadow/radius/spacing value inputs — → `PUT` → live overlay; static read-only fallback when DB off) | Editing any token in Appearance writes to DB and updates the app |
| Token emit pipeline (JSON + CSS) | Service / Feature | Compiles DB tokens to a JSON map + a **Tailwind-compatible CSS** (`@theme` / `:root` CSS vars) the app + utilities consume | Backend `emit.py`: `tokens` → `/api/tokens.json` + `/api/tokens.css` (`@theme` or runtime `:root`); app overlays the `:root` build at boot + after edits | tokens table + backend | L | ✅ (`emit.py` → JSON + CSS, recomputed per request (`Cache-Control: no-cache`, so edits are instant); `db-tokens.ts` overlays `:root` at boot + on edit; `:root` scope also emits `shadow-*` utility overrides) | DB tokens → JSON + Tailwind `@theme`/`:root` CSS; utilities resolve from emitted vars; edits apply without rebuild |

> **Figma token sync** is deferred to the **final phase** (see "Recommended migration order" #11) — it's a foundations *refresh loop* layered on top of the now-complete token pipeline, not a prerequisite for any other work.
...
| AI chat panel (3 modes) | Pattern / Block | Compact/chat/sidebar assistant | Done — modes switch, **live inference**, and a **tool loop that injects/edits widgets** (fetch/search/callApi/listConnections/injectJson/injectConnection/addWidget/listWidgets/updateWidget/removeWidget/setContext). Streamed replies persist; agent has content-level context + anti-fabrication/dedup/narration guards. Conversations persist to DB | backend + DB convos | L | ✅ (live AI + injection) | Modes switch; live replies; injects widgets |
...
| Widget picker / editor | Pattern / Block | Add/edit widgets — structured Settings tab + **Design tab** | Done. Picker = categorized catalog. Editor (slideout) now has a **structured Settings tab** (legacy `DataConnectionModal` parity: name, widget-type, data-source incl. custom one-off REST, request editor + **Test/Load fields**, field mapping, per-column format/animation, content editors, footer citation + link-to-page, move-to-page, delete) **and** a **Design tab** (per-element token-bound inspector) | — | M | ✅ (Settings + Design) | Add + edit widgets; both tabs present |
...
| Item | Type | What it Does | How to Migrate | Prerequisites | Complexity | Status | Success Criteria |
|---|---|---|---|---|---|---|---|
| `tokens` | Data model (DB) | Design tokens (color/type/spacing/radius/shadow) | New table; seed from Untitled `@theme`; serve as CSS vars | Postgres + backend | L | ✅ (typed table live — **555 tokens** (501 color incl. `--color-chart-1..6`, 29 typography, 14 shadow, 10 radius, 1 spacing) seeded from theme.css; `GET/PUT /api/tokens` upserts any kind; JSON/CSS emit + live overlay; **all kinds editable** from Appearance, verified by tests) | Tokens CRUD; app reads from DB; every kind editable |
| `components` | Data model (DB) | Atom/molecule/pattern defs widgets pull from | New table (id, kind, variants, sizes, token refs); registry resolves by id; **cached as JSON** | Postgres + registry | XL | 🟡 (catalog: `src/mip/data/components.json` (16 atoms/molecules) + `components-catalog.ts` (`loadComponents()` DB overlay) + `seed_components`. **Registry resolution landed**: a generic `element` widget renders any atom by `settings.componentId` (button/badge/avatar/checkbox/toggle/input/progressBar) with catalog-default variant/size; the picker offers an **Elements** group sourced from the catalog. Molecule/pattern resolution + token-bound variant editing still pending) | Widgets render from DB component rows; JSON cache busts on edit |
...
| `apps` / `installed_apps` | Data model (DB) | Connector catalog + install state | Move `apps-catalog.ts` → DB; **cached as JSON** | Postgres | M | 🟡 (catalog moved to canonical `src/mip/data/apps.json`; `apps-catalog.ts` imports it + `loadApps()` overlays DB rows; server `seed_apps` seeds the `apps` collection from the same file. Install state + UI-on-loader still localStorage) | Catalog + install state from DB; JSON cache |
...
| `themes` / `templates` | Data model (DB) | Palettes + starter dashboards | JSON → DB; **cached as JSON** | Postgres | M | 🟡 (templates moved to canonical `src/mip/data/templates.json`; `templates-catalog.ts` imports it + `loadTemplates()` overlays DB rows; server `seed_templates` seeds the `templates` collection. Themes + UI-on-loader still pending) | From DB; JSON cache |
...
| AI tool catalog | Service / Flow | The assistant's tools, surface-scoped via a registry | Built — live loop runs: `listConnections` · `callApi` · `fetch` · `search` · `listWidgets` · `injectJson` · `injectConnection` · `addWidget` · `updateWidget` · `removeWidget` · `setContext` · `loadSkill` · `describeTool` (canvas surface has its own DOM tools). Strict-REST guard nudges `injectConnection` for API data; untrusted-data/trust-boundary handling. Still missing vs mip: `ai_request` (second-opinion), `generate_image`, `memory_*`, and JSONPath-against-live-sample validation pre-inject. | injection engine + backend | L | 🟡 (core tool set live; image/memory/second-opinion ⬜) | Assistant runs the full tool set with the same gates as mip |
...
| Web search tool | Service | `search` (e.g. Tavily) for in-chat research | Done — `search` tool runs via a Tavily connection the user adds; results feed widget building and citations. (Key still client-side until secret storage lands.) | backend + key | S | ✅ (Tavily) / 🟡 (key storage) | Assistant can web-search and cite |
...
| Item | Type | What it Does | How to Migrate | Prerequisites | Complexity | Status | Success Criteria |
|---|---|---|---|---|---|---|---|
| Widget Design tab | Feature / Flow | Per-widget **Design** tab in settings — edit color/spacing/font/layout of the widget **and the elements/components nested inside it**, bound to tokens (pick a token or override) | Done — Figma-style inspector with **per-element targeting tabs** (card/title/value/axis/header/cell/…): Color (text/sub-text/accent w/ a token-swatch picker from `COLOR_TOKEN_GROUPS`), Fill, Stroke (color/width/style/radius), Text (size/weight/align/spacing), Effects (shadow/opacity), Spacing, and raw Custom CSS. Overrides save to `widget.style` / per-element style and emit scoped CSS; empty fields inherit the theme. DB-component inheritance still pending. | tokens + components in DB; widget editor | L | ✅ (token-swatch + overrides) / 🟡 (DB component inheritance) | Changing a control restyles the widget (and chosen nested element) live and persists |
...
| Live widget data | Feature / Flow | Widgets fetch from connections | Done — `useWidgetData` resolves `widget.data` (sourceId+request+map) → backend proxy → JSONPath (`readJsonPath`). KPI honors value/delta map (+ auto-detect of common delta fields). **Collection widgets bind live** via `injectConnection` + `map` (`series`/`items`/`rows`/`record`) — charts/list/table/detail render from the mapped array; editor exposes the mapping + Test. Auto-sort/paginate from live sources still TODO. | connections + backend | L | ✅ (KPIs + collections live) / 🟡 (sort/paginate) | Widget shows live API data |
...
| Table cell formats + animations | Feature / Flow | Per-column cell-format + animation for table/detail | Done (core set) — `formatCell` + `CELL_FORMATS` (auto/text/number/currency/percent/date/dateTime/month/year/duration) and `CELL_ANIMATIONS` (none/flash-on-change/pulse); set per-column in the editor's Columns/Fields editor and applied in the table & detail renderers. Full 29-format / 9-animation parity still TODO. | table widget | S | ✅ (core set) / 🟡 (full parity) | Cells format/animate per id |
| Widget footer (citation + Read-more) | Feature / Flow | Every widget shows a footer: source citation link (left) + Read-more page link (right); hidden when neither set | Done — `WidgetChrome` renders `settings.caption`/`captionHref` (auto-linkified URLs) as a citation and `details.route` as a Read-more that navigates to another page; editor exposes both; agent sets caption/captionHref to the data source when it fetches/searches | widget chrome | S | ✅ | Footer shows source + page link; hidden if empty |
| Link to page (widget → page nav) | Feature / Flow | A widget links to another dashboard page (Read-more) | Done — `details.route` = target page id; footer Read-more calls `setActivePage`. (External-URL link-to still optional.) | pages | S | ✅ (page nav) | Read-more navigates to the linked page |
| Generate system prompt with AI | Feature / Flow | "Write this for me" button on the per-page AI-context field | Done — Dashboard Settings → AI assistant context has a sparkle button that sends the title + current draft to the model and fills in a polished system prompt | chat/model | S | ✅ | Button generates a usable assistant context |
| Agent robustness | Feature / Flow | Keep the assistant acting, accurate, and non-repetitive | Done — no fixed step limit (repetition guard), nudges on pushback / empty / "let me search…" intent, content-level context of existing widgets, RELEVANCE + NO-DUPLICATE + DATA-INTEGRITY (no fabricated stats) rules, source citations, streamed-reply persistence | agent loop + prompt | M | ✅ | Agent acts instead of narrating; no fabricated/duplicate widgets |
...
|---|---|---|---|---|
| **App shell** | `/` | Three columns: collapsible left **Sidebar** (w-60) · center column (Topbar + main, `min-w-0`) · optional right **AI chat** panel (w-80). Dark Untitled theme. | Sidebar collapses to width-0 (animated); chat panel pushes content as an in-flow sibling and the grid reflows + clips so widgets never overlap it; everything themed by tokens. | ✅ |
...
| **Widget editor** | edit-mode pencil | Untitled `SlideoutMenu` drawer with **Settings** + **Design** tabs. Settings = name, widget-type, **data source** (inline / custom one-off REST / saved connection), request editor (method/path/params/headers/body + **Test / Load fields**), structured **field mapping**, **Columns/Fields** editor (per-column format + animation), **content editors** (markdown/diagram/image), **Footer** (source citation + Link-to-page), collapsible **Widget settings** + **Advanced (JSON)**, **Move to page**, **Delete**. Design = per-element token-bound inspector. | Edit any aspect of the widget → Save persists via store; Test runs the live request and offers detected fields as autocomplete; invalid JSON shows a path error. | ✅ (Settings + Design) |
...
| **AI chat — sidebar** | topbar sparkle | Full-height right panel (w-80, border-l): header shows the **active connection · model** or "No AI connection"; greeting; message list (user bubbles right, assistant markdown left w/ avatar, tool entries with collapsible result JSON); composer ("Ask anything…" + send/stop) with an inject-mode toggle + mic; footer toolbar = compact/chat/sidebar toggles + **settings gear**. | Type + Enter to send; context-aware suggestion chips when fresh; streaming reply + "thinking…"; **runs the tool loop to build/edit the dashboard** (live when an AI connection is set, else demo responder); Stop aborts; transcript persists per page to DB; gear → Assistant Settings popover. | ✅ (live AI + tools) |
...
| **Dashboard Settings · General** | per-page settings (topbar gear / page menu) | Modal "Dashboard Settings" with left tabs (General · Access · Dynamic Variables). General = "Page Settings": Title `Input`, **Page ID** (e.g. `new-page-17-5zdir`), Description `TextArea`, **Layout Mode** `Select` (Dashboard sidebar+topbar / Fullpage) with helper, **AI assistant context (system prompt)** `TextArea` with a **sparkle "write this for me with AI"** button (generates a polished context from the title + current draft) and a default-prompt placeholder. Footer: Cancel / Save Settings. | Edit page identity, layout mode, and per-page assistant context; the sparkle button calls the model to draft the prompt; Save persists to the page via `store.updatePageSettings` (→ DB later). | ✅ |
...
11. **Figma token sync** — the **final phase** (foundations refresh loop): Figma MCP / Tokens Studio export → `tokens` upsert job → re-emits JSON + CSS. Layered on the finished token pipeline; depends on nothing else.
