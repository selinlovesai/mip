...
| **Topbar** | top of `/` | Left: chevron-right reopen (when collapsed) + "Page / <title>". Right: theme toggle (sun/moon), **Dashboard-settings gear (per-page)**, Add-widget `+`, Edit-mode lock, AI sparkle — all Untitled `ButtonUtility`. (mip also has responsive/fullscreen/save/grid controls.) | Toggles light/dark; **gear opens the per-page Dashboard Settings modal** (NOT system-wide settings — those are in the sidebar user menu); `+` opens widget picker; lock toggles edit mode; sparkle opens chat. ⚠️ mip-tailwind currently wires the gear to the global Settings surface — should be repointed to Dashboard Settings. | 🟡 (gear target to fix) |
...
| **Settings shell (system-wide)** | **sidebar user menu → Settings** | Dedicated surface with its **own inner sidebar** (Profile · Appearance · Connections · Apps · Assistant · Access · Users) + content pane; outer workspace sidebar/topbar stay. Back arrow returns to dashboard. | Open from the account/user menu (not the topbar gear); click a tab → content swaps; back arrow closes. ⚠️ mip-tailwind opens this from the topbar gear today — should move to the user menu. | ✅ (UI) / 🟡 (entry point) |
